...
// const provider = new ethers.providers.JsonRpcProvider('https://metis-pokt.nodies.app');
const provider = new ethers.providers.JsonRpcProvider(
  "https://metis-mainnet.g.alchemy.com/v2/FWmhvca-2KGl6D1o9YcToyEeO8Lmshcy"
);
// const provider = new ethers.providers.JsonRpcProvider('https://metis-mainnet.g.alchemy.com/v2/pr6aJGlwxiBllJDEBLsr54-0N9KiX10T');
...
async function main() {
  ...
  const stakers = JSON.parse(fs.readFileSync("./uniqueStakers.json", "utf8"));

  const uniqueAddresses = new Set();

  accounts.forEach((holder) => uniqueAddresses.add(holder));
  stakers.forEach((holder) => uniqueAddresses.add(holder));

  const uniqueAccounts = Array.from(uniqueAddresses);
  console.log("🚀 ~ main ~ uniqueAccounts:", uniqueAccounts);
  ...
  await processTokenRewards(Multicall, uniqueAccounts, gauges, outputFile);
}
...
async function batchMulticall(Multicall, calls, maxRetries = 10) {
  ...
  for (let i = 0; i < calls.length; i += BATCH_SIZE) {
    ...
    while (attempts < maxRetries && !success) {
      console.log("🚀 ~ batchMulticall ~ attempts:", i, attempts);
      try {
    ...
    console.log("🚀 ~ batchMulticall ~ results from", i, "to", i + BATCH_SIZE);
  }
  return results;
}
...
async function processTokenRewards(
  ...
  const rewardCalls = gaugeAddresses.flatMap((gauge) =>
    accounts.map((account) => ({
      target: gauge,
      callData: new ethers.utils.Interface(GAUGE_ABI).encodeFunctionData(
        "earned",
        [hermes, account]
      ),
    }))
...
function processAndFormatResults(
  ...
  for (const gaugeAddress of gaugeAddresses) {
    ...
    for (const account of accounts) {
      const pendingReward = ethers.utils.defaultAbiCoder
        .decode(["uint256"], returnData[index])[0]
        .toString();
      console.log(
        "🚀 ~ processAndFormatResults ~ pendingReward:",
        pendingReward
      );
